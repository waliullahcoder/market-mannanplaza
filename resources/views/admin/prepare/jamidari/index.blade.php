                                ...
                                <div class="row">
                                    <div class="col-md-4 col-6 offset-md-4">
                                        <a style="font-size: 16px;background: none;border: 1px solid;"
                                            ...
                                            href="{{ route('jamidari.prepare.add.individual') }}">
                                    ...
                                    <div class="col-md-4 col-6">
                                        <a style="font-size: 16px;background: none;border: 1px solid;"
                                            class="text-center btn-block btn-outline-info bg-none btn-lg buttonAddEdit "
                                            href="{{ route('jamidari.prepare.add') }}">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data->jamidari_list as $jamidari)
                                        <tr class="row_{{ $jamidari->SerialNo }}">
                                            ...
                                            <td>{{ @$jamidari->position_holder->Floor }}</td>
                                            <td>{{ Carbon\Carbon::parse($jamidari->PaidDate)->format('d-m-Y') }}
                                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                                {{ Carbon\Carbon::parse($jamidari->PaidDate)->format('(l)') }}
                                            </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#datatable tbody').on('click', 'i.fa-trash', function() {
                ...
                let serialNo = $(this).parent().data('id');
                var tableRow = this;
                swal({
                        title: "Are you sure?",
                    ...
                    function(isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                type: "POST",
                                url: "{{ route('jamidari.prepare.delete') }}",
                                data: {
                                    serialNo: serialNo
                                },
