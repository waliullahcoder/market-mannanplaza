            ...
            <table id="dataTable" class="table table-bordered table-striped"  name="areaTable">
                ...
                <tbody id="">
                	...
                	@foreach ($allUtilities as $utility)
                		<tr class="row_{{ $utility->id }}">
                			<td>{{ $sl++ }}</td>
                			<td>{{ $utility->code }}</td>
                            <td>{{ $utility->name }}</td>
                			<td>
                                @php
                                    echo \App\Link::status($utility->id,$utility->status);
                                @endphp
                			</td>
                			<td>
                    			@php
                    				echo \App\Link::action($utility->id);
                    			@endphp                				
                			</td>
                		</tr>
                	@endforeach
...
@section('custom-js')
    <script>
        ...
        function statusChange(utilityId) {
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: "post",
                url: "{{ route('utilitySetup.status') }}",
                data: {utilityId:utilityId},
                success: function(response) {
                    swal({
                        title: "<small class='text-success'>Success!</small>", 
