            ...
            <table id="dataTable" class="table table-bordered table-striped"  name="areaTable">
                ...
                <tbody id="">
                	...
                	{{-- @foreach ($allFloors as $floor)
                		<tr class="row_{{ $floor->id }}">
                			<td>{{ $sl++ }}</td>
                			<td>{{ $floor->code }}</td>
                            <td>{{ $floor->name }}</td>
                			<td>
                                @php
                                    echo \App\Link::status($floor->id,$floor->status);
                                @endphp
                			</td>
                			<td>
                    			@php
                    				echo \App\Link::action($floor->id);
                    			@endphp                				
                			</td>
                		</tr>
                	@endforeach --}}
...
@section('custom-js')
    <script>
        ...
        function statusChange(floorId) {
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: "post",
                url: "{{ route('floorSetup.status') }}",
                data: {floorId:floorId},
                success: function(response) {
                    swal({
                        title: "<small class='text-success'>Success!</small>", 
